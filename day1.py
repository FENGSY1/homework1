...
data = pd.read_csv(
    'E:\\学业\\aiSummerCamp2023-master\\day1\\assignment\\data\\train.csv')
df = data.copy()
df.sample(10)
# %%
# 去除无用特征
...
models['KNeighbor'] = KNeighborsClassifier(
    n_neighbors=5)  # n_neighbors表示neighbor个数
models['RandomForest'] = RandomForestClassifier(
    n_estimators=100)  # n_estimators表示树的个数
# %%
...
def plot_cm_ratio(model, y_true, y_pred, name=None):
    ...
    disp = ConfusionMatrixDisplay(
        confusion_matrix=cm_ratio, display_labels=model.classes_)
    disp.plot(ax=ax)
    plt.show()
    return None
